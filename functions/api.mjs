/**
 * Vittoria HR — cloud bridge as a Cloudflare Pages Function.
 * Route: POST https://<project>.pages.dev/api
 *
 * Same wire protocol as netlify/functions/api.mjs. Env vars are read from
 * Pages project settings (Settings → Environment variables) via context.env:
 *   DATABASE_URL   — required (Netlify DB / Neon / any Postgres)
 *
 * @neondatabase/serverless uses its HTTP driver in Workers — no TCP needed.
 */
...
export async function onRequest(context) {
  ...
  if (!request.headers.get("x-vittoria-session")) return json({ ok: false, error: "Sesi tidak ditemukan." }, 401);

  const DATABASE_URL = env.DATABASE_URL;
  if (!DATABASE_URL) {
    return json({ ok: false, error: "DATABASE_URL belum ada — set di Cloudflare Pages → Settings → Environment variables." }, 500);
  }

  const sql = neon(DATABASE_URL);
  let body;
  try { body = await request.json(); } catch { return json({ ok: false, error: "Body tidak valid." }, 400); }
  ...
  try {
    ...
    if (op === "pull") {
      ...
      for (const s of wanted) {
        ...
        data[s.key] = raw.map((row) => {
          const out = {};
          for (const [p, c, k] of s.cols) out[p] = toProp(row[c], k);
          return out;
        });
        counts[s.key] = data[s.key].length;
    ...
    if (op === "sync" || op === "remove" || op === "clear") {
      ...
      if (op === "sync") {
        ...
        const CHUNK = Math.max(1, Math.floor(60000 / spec.cols.length));
        let affected = 0;
    ...
    if (op === "stats") {
      ...
      return json({ ok: true, rows: total, tables, version, revs, counts, server_version: ver, presence_active: pres.map(presenceRow) });
    }
    ...
    if (op === "ping") {
      ...
      return json({
        ...
        tables, missing: missing.slice(0, 5), rows: rowsTotal, server_ms: Date.now() - t0,
      });
    }
