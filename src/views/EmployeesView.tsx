/**
 * Pengguna (Admin) — directory + single-step account wizard (data only).
 * The base photo is NOT asked here: the employee takes it on first login.
 */
import { useMemo, useState } from "react";
import { useApp } from "../lib/store";
import { EMAIL_DOMAIN, EMAIL_RE, Employee, EmpStatus, genPassword, nextStaffId, ROLE_LABEL, Role, STATUS_LABEL } from "../lib/database";
...
export default function EmployeesView() {
  ...
  const filtered = useMemo(
    () => employees.filter((e) =>
      ...
      (e.name.toLowerCase().includes(search.toLowerCase()) || e.staffId.toLowerCase().includes(search.toLowerCase()) || e.email.toLowerCase().includes(search.toLowerCase()))),
    [employees, search, dept],
  );

  const today = todayKey();
  const presentToday = (staffId: string) => logs.some((l) => l.staffId === staffId && l.type === "IN" && l.status === "VERIFIED" && wibDayKey(new Date(l.ts)) === today);
  ...
  const [department, setDepartment] = useState("Gudang");
  const [role, setRole] = useState<Role>("employee");
  const [shift, setShift] = useState("sh-pagi");
  const [fSite, setFSite] = useState<string | null>(null);
  const [err, setErr] = useState("");

  const startWizard = () => {
    setOpen(true);
    setName(""); setStaffId(nextStaffId(employees)); setEmail(""); setEmailTouched(false);
    setPassword(genPassword()); setDepartment(departments[0] ?? "Gudang"); setRole("employee");
    setShift("sh-pagi"); setFSite(activeSite?.id ?? null); setErr("");
  };
  ...
  const createAccount = () => {
    ...
    const mail = (emailTouched ? email : suggestedEmail).trim().toLowerCase();
    if (!EMAIL_RE.test(mail)) return setErr("Email tidak valid.");
    if (employees.some((e) => e.email.toLowerCase() === mail)) return setErr("Email sudah terdaftar.");
    const isCentral = role === "superadmin" || role === "companyadmin";
    const sd = salaryDefaults[role];
    const emp: Employee = {
      staffId: staffId.trim(), nik: `3171${String(Math.floor(100000000 + Math.random() * 899999999))}`,
      name: name.trim(), email: mail, password,
      phone: "+62 812-0000-0000", address: "—", emergencyName: "—", emergencyPhone: "—",
      department, position: role === "employee" ? "Staff Gudang" : ROLE_LABEL[role],
      role, shiftId: shift, status: "active",
      salary: { ...sd }, siteId: isCentral ? null : fSite,
      photo: null, descriptor: null, hash: null,   // base photo: diambil saat login pertama
      deviceId: null, deviceBoundAt: null, createdAt: Date.now(),
    };
    addEmployee(emp);
    audit("USER_CREATE", emp.staffId, `Akun ${emp.name} (${department} · ${ROLE_LABEL[role]}) dibuat — foto menyusul saat login pertama`);
    toast.push("ok", "Akun berhasil dibuat", `${emp.name} akan mengambil foto tanda tangannya saat login pertama.`);
  ...
  const [eForm, setEForm] = useState<Partial<Employee>>({});
  const openEdit = (e: Employee) => { setEdit(e); setEForm({ name: e.name, nik: e.nik, phone: e.phone, address: e.address, emergencyName: e.emergencyName, emergencyPhone: e.emergencyPhone, department: e.department, position: e.position, shiftId: e.shiftId, status: e.status, siteId: e.siteId, role: e.role }); };
  const saveEdit = () => {
    if (!edit) return;
  ...
  return (
    <div className="space-y-5 pb-2">
      <div className="flex items-start justify-between">
        ...
        <button className="btn-sun !rounded-xl !px-4 !py-2.5 text-sm" onClick={startWizard}><IconPlus size={16} /> Akun</button>
      </div>
        ...
        <section className="card anim-fade-up overflow-hidden">
          <div className="flex items-center justify-between border-b border-ink-100 bg-ink-50 px-4 py-3">
            <div className="flex items-center gap-2.5">
              <span className="grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-sun-400 to-sun-600 text-white shadow-[0_4px_12px_rgba(240,115,0,0.35)]"><IconPlus size={16} /></span>
              <div>
                <p className="font-display text-[15px] leading-tight font-extrabold text-ink-900">Buat Akun Karyawan</p>
                <p className="text-[10.5px] font-bold text-ink-400">Cukup data diri & kredensial — tanpa kamera</p>
              </div>
            </div>
            <button onClick={() => setOpen(false)} className="cursor-pointer rounded-lg p-1.5 text-ink-400 hover:bg-ink-100" aria-label="Tutup"><IconX size={15} /></button>
          </div>
          <div className="space-y-3.5 p-4">
            ...
            <div>
              ...
              <div className="field-wrap">
                ...
                <input type="email" className="input !py-3 font-mono !text-[13.5px]" placeholder={`nama@${EMAIL_DOMAIN}`} value={suggestedEmail} onChange={(e) => { setEmail(e.target.value); setEmailTouched(true); }} />
              </div>
            ...
            <div>
              ...
              <select className="input" value={fSite ?? ""} onChange={(e) => setFSite(e.target.value || null)}>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="grid h-12 w-12 shrink-0 cursor-pointer place-items-center rounded-xl border border-ink-100 bg-white text-ink-500 transition active:scale-90" onClick={() => setPassword(genPassword())} aria-label="Acak ulang kata sandi" title="Acak ulang"><IconRefresh size={16} /></button>
              </div>
            </div>
            <p className="rounded-xl bg-sky-100/70 px-3 py-2 text-[11.5px] leading-relaxed font-semibold text-sky-600">
              <b>Tanpa foto di sini.</b> Saat karyawan login pertama kali, aplikasi menampilkan langkah foto tanda tangan — perangkatnya sekaligus diikat otomatis ke akunnya.
            </p>
            {err && <Banner tone="warn">{err}</Banner>}
            <button className="btn-sun w-full" onClick={createAccount}><IconCheck size={16} /> Buat Akun</button>
          </div>
        </section>
      )}
      ...
      <div className="space-y-2.5">
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-0.5">
          {["Semua", ...departments].map((d) => (
            <button key={d} onClick={() => setDept(d)} className={`shrink-0 cursor-pointer rounded-full px-3.5 py-1.5 text-[12px] font-extrabold transition ${dept === d ? "bg-ink-900 text-white shadow" : "border border-ink-100 bg-white text-ink-500 hover:border-ink-200"}`}>{d}</button>
          ))}
        </div>
      </div>
      ...
      {/* directory */}
      {filtered.length === 0 ? (
        <EmptyState icon={<IconUsers size={26} />} title="Tidak ada pengguna ditemukan" desc="Ubah kata kunci atau buat akun baru."
          action={<button className="btn-sun !px-4 !py-2.5 text-sm" onClick={startWizard}><IconPlus size={15} /> Buat Akun</button>} />
      ) : (
        <div className="space-y-2.5">
          {filtered.map((e, i) => (
            <div key={e.staffId} className={`tile-pop card card-press flex items-center gap-3 p-3.5 ${e.status !== "active" ? "opacity-60" : ""}`} style={{ animationDelay: `${i * 40}ms` }}>
              <div className="relative">
                ...
                <span className={`absolute -right-1 -bottom-1 h-3.5 w-3.5 rounded-full border-2 border-white ${presentToday(e.staffId) ? "bg-ok-500" : "bg-ink-200"}`} title={presentToday(e.staffId) ? "Hadir hari ini" : "Belum hadir"} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="flex flex-wrap items-center gap-1.5 text-[14.5px] font-extrabold text-ink-900">
                  <span className="truncate">{e.name}</span>
                  <Chip tone={e.role === "superadmin" ? "grape" : e.role === "companyadmin" ? "sun" : e.role === "manager" ? "sky" : "ink"} className="!px-1.5 !py-0.5 !text-[9px]">{ROLE_LABEL[e.role].toUpperCase()}</Chip>
                  {e.status !== "active" && <Chip tone="danger" className="!px-1.5 !py-0.5 !text-[9px]">{STATUS_LABEL[e.status].toUpperCase()}</Chip>}
                  {e.descriptor || e.hash ? <Chip tone="ok" className="!px-1.5 !py-0.5 !text-[9px]"><IconFace size={9} /> {e.descriptor ? "128-D" : "LITE"}</Chip>
                    : <Chip tone="warn" className="!px-1.5 !py-0.5 !text-[9px]"><IconCamera size={9} /> BELUM FOTO</Chip>}
                  {e.deviceId && <Chip tone="teal" className="!px-1.5 !py-0.5 !text-[9px]"><IconSmartphone size={9} /> TERIKAT</Chip>}
                </p>
                <p className="font-mono text-[11px] font-semibold text-ink-400">{e.staffId} · {e.department} · {sites.find((s) => s.id === e.siteId)?.shortName ?? "Pusat"} · {relTime(e.createdAt)}</p>
                <p className="truncate font-mono text-[10.5px] font-semibold text-ink-300">{e.email}</p>
              </div>
              <div className="flex shrink-0 items-center gap-1">
                {isSuper && e.deviceId && (
                  <button className="grid h-9 w-9 cursor-pointer place-items-center rounded-xl bg-teal-100/70 text-teal-600 transition hover:bg-teal-100 active:scale-90"
                    onClick={() => { unbindDevice(e.staffId); toast.push("info", "Ikatan perangkat dilepas", `${e.name} dapat login dari perangkat baru.`); }}
                    aria-label={`Lepas perangkat ${e.name}`} title="Lepas ikatan perangkat"><IconSmartphone size={15} /></button>
                )}
                <button className="grid h-9 w-9 cursor-pointer place-items-center rounded-xl bg-ink-50 text-ink-400 transition hover:bg-grape-100 hover:text-grape-600 active:scale-90"
                  onClick={() => {
                    const p = genPassword();
                  ...
                  aria-label={`Reset kata sandi ${e.name}`} title="Reset kata sandi"><IconLock size={15} /></button>
                <button className="grid h-9 w-9 cursor-pointer place-items-center rounded-xl bg-ink-50 text-ink-400 transition hover:bg-sky-100 hover:text-sky-600 active:scale-90"
                  onClick={() => openEdit(e)} aria-label={`Edit ${e.name}`} title="Edit profil"><IconEdit size={15} /></button>
                <ConfirmButton label="" icon={<IconTrash size={15} />} confirmLabel="Hapus?"
      ...
      <Modal open={!!credModal} onClose={() => setCredModal(null)} title="Kredensial Login">
        {credModal && (
          <div className="space-y-3">
            <Banner tone="warn" title="Tampilkan sekali!">Serahkan kredensial ini langsung ke {credModal.name}. Foto tanda tangan akan diambil sendiri oleh karyawan saat login pertama.</Banner>
        ...
        {edit && (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between rounded-2xl border border-ink-100 px-3.5 py-2.5">
              <p className="text-[12px] font-extrabold text-ink-700">Foto tanda tangan</p>
              <Chip tone={edit.descriptor || edit.hash ? "ok" : "warn"}>
                <IconFace size={11} /> {edit.descriptor ? "128-D tersimpan" : edit.hash ? "dHash tersimpan" : "menunggu login pertama"}
              </Chip>
            </div>
      ...
      <span className="hidden"><Toggle checked={false} onChange={() => undefined} /></span>
    </div>
  );
}
