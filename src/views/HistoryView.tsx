 ...
 * slip, filters, CSV/Excel export, photo evidence modal, and a
 * geofence map/radar of the month's check-ins.
 */
...
import { IconCalendar, IconCamera, IconCheck, IconDownload, IconWallet, IconX } from "../components/icons";

type CellKind = "work" | "late" | "leave" | "absent" | "holiday" | "future" | "empty";
...
const WEEKDAYS = ["Sen", "Sel", "Rab", "Kam", "Sab", "Min"];
...
export default function HistoryView() {
  ...
  /* --------------------------- calendar --------------------------- */
  const cal = useMemo(() => {
    const days = monthDays(month); // ["2026-02-01", …]
    const first = new Date(`${month}-01T00:00:00`);
    const lead = (first.getDay() + 6) % 7; // Monday-first
    const cells: Array<{ day: string | null; kind: CellKind; label: string }> = [];
    for (let i = 0; i < lead; i++) cells.push({ day: null, kind: "empty", label: "" });
    for (let d = 1; d <= days.length; d++) {
      const key = days[d - 1];
      const s = daySummary(staffId, key, logs, breaks, leaves, shifts, shiftId, company.holidays);
      let kind: CellKind;
      if (key > today) kind = "future";
      else if (s.inTs) kind = s.lateMin > 0 ? "late" : "work";
      else if (s.kind === "holiday") kind = "holiday";
      else if (s.kind === "leave") kind = "leave";
      else kind = "absent";
      cells.push({ day: key, kind, label: String(d) });
    }
    return cells;
  }, [month, staffId, logs, breaks, leaves, shifts, shiftId, company.holidays, today]);

  const monthStats = useMemo(() => {
    const real = cal.filter((c) => c.day && c.kind !== "future" && c.kind !== "empty");
    ...
    let totalWork = 0, totalLate = 0, totalBreak = 0, totalOt = 0;
    for (const c of real) {
      if (!c.day) continue;
      const s = daySummary(staffId, c.day, logs, breaks, leaves, shifts, shiftId, company.holidays);
      totalWork += s.workMin;
      totalLate += s.lateMin;
      totalBreak += s.breakMin;
      totalOt += s.overtimeMin;
    }
    return { hadir, terlambat, cuti, libur, absen, totalWork, totalLate, totalBreak, totalOt };
  }, [cal, staffId, logs, breaks, leaves, shifts, shiftId, company.holidays]);
  ...
  const exportXlsx = async () => {
    ...
    const rows = filtered.map((l) => ({
      Tanggal: wibShortDate(new Date(l.ts)), Jam: wibTime(new Date(l.ts)), "Staff ID": l.staffId,
      Nama: l.name, Tipe: l.type, "Jarak (m)": l.distanceM, Status: l.status, Metode: l.method,
      Alasan: l.reason ?? "",
    }));
  ...
  const monthPoints = logs.filter((l) => l.staffId === staffId && wibDayKey(new Date(l.ts)).startsWith(month));
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-1.5">
          <button className="btn-soft !rounded-xl !px-3 !py-2.5 text-[12px]" onClick={exportCsv} disabled={!filtered.length}>CSV</button>
          <button className="btn-soft !rounded-xl !px-3 !py-2.5 text-[12px]" onClick={() => void exportXlsx()} disabled={!filtered.length}>Excel</button>
        </div>
      </div>
      ...
      <section className="card p-4">
        <SectionLabel
          right={
            <input
              type="month"
              className="input !w-auto !py-1.5 !px-2.5 text-[13px]"
              value={month}
              max={today.slice(0, 7)}
        ...
        >
          <span className="inline-flex items-center gap-1.5"><IconCalendar size={17} /> Kalender</span>
        </SectionLabel>
        <div className="grid grid-cols-6 gap-1.5">
          {WEEKDAYS.map((w) => (
            <span key={w} className="py-1 text-center text-[10px] font-extrabold tracking-wide text-ink-400 uppercase">{w}</span>
          ))}
          {cal.map((c, i) => (
            <div
              key={i}
              className={`grid aspect-square place-items-center rounded-xl border text-[13px] font-extrabold transition ${cellStyle[c.kind]} ${c.day === today ? "ring-2 ring-sun-400" : ""}`}
              title={c.day ? `${c.day} · ${c.kind}` : undefined}
            >
              {c.label || ""}
            </div>
          ))}
        </div>
        <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1.5 text-[10px] font-extrabold text-ink-400">
      ...
      <section className="space-y-2.5">
        ...
        <div className="card overflow-hidden">
          <div className="flex items-center justify-between border-b border-dashed border-ink-200 bg-ink-900 px-4 py-3">
            <div>
              <p className="flex items-center gap-1.5 font-display text-[15px] font-extrabold text-white"><IconWallet size={15} className="text-sun-300" /> Slip Jam Kerja</p>
      ...
      {/* ledger */}
      <section>
        <SectionLabel right={<Chip tone="ink">{filtered.length} catatan</Chip>}>Catatan Absen</SectionLabel>
        <div className="mb-2.5 flex gap-1.5 overflow-x-auto pb-0.5">
          {["Semua", "IN", "OUT"].map((t) => (
            <button key={t} onClick={() => setType(t)} className={`shrink-0 cursor-pointer rounded-full px-3.5 py-1.5 text-[12px] font-extrabold transition ${
              type === t ? "bg-ink-900 text-white shadow" : "border border-ink-100 bg-white text-ink-500"
            }`}>{t === "IN" ? "Check-In" : t === "OUT" ? "Check-Out" : "Semua"}</button>
          ))}
          {["Semua", "VERIFIED", "REJECTED"].map((s) => (
            <button key={s} onClick={() => setStatus(s)} className={`shrink-0 cursor-pointer rounded-full px-3.5 py-1.5 text-[12px] font-extrabold transition ${
              status === s ? "bg-sun-500 text-white shadow" : "border border-ink-100 bg-white text-ink-500"
            }`}>{s === "VERIFIED" ? "Terverifikasi" : s === "REJECTED" ? "Ditolak" : "Status: Semua"}</button>
          ))}
        </div>
              ...
              <div key={l.id} className="flex items-center gap-3 px-3.5 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex flex-wrap items-center gap-1.5 text-[13px] font-extrabold text-ink-900">
                    ...
                    <Chip tone={l.status === "REJECTED" ? "danger" : "ok"} className="!px-1.5 !py-0.5 !text-[9px]">{l.status === "REJECTED" ? "DITOLAK" : "OK"}</Chip>
                    {l.lateMin ? <Chip tone="warn" className="!px-1.5 !py-0.5 !text-[9px]">TELAT {l.lateMin}m</Chip> : null}
                    {l.overtimeMin ? <Chip tone="sun" className="!px-1.5 !py-0.5 !text-[9px]">LEMBUR {l.overtimeMin}m</Chip> : null}
                  </p>
                  <p className="truncate text-[11px] font-semibold text-ink-400">
                    {wibShortDate(new Date(l.ts))} · {formatMeters(l.distanceM)} · {l.method === "face" ? "wajah" : "manual"}
                    {l.reason ? ` · ${l.reason}` : ""}
                  </p>
                </div>
                <span className="font-mono text-[12.5px] font-bold text-ink-600 tabular-nums">{wibTime(new Date(l.ts))}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <span className="hidden"><IconCheck size={1} /><InitialsAvatar name="" seedKey="" size="hidden" /></span>
    </div>
  );
}
