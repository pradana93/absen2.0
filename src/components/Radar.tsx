/** Geofence radar — SVG map: HQ, rings, sweep, plots, live position. */
...
interface Props { hq: GeoPoint; radiusM: number; points: AttendanceLog[]; live: GeoReading | null; }
...
export default function Radar({ hq, radiusM, points, live }: Props) {
  ...
  const maxDist = useMemo(() => {
    ...
    if (live && (live.status === "locked" || live.status === "sim")) m = Math.max(m, haversineMeters(live, hq) * 1.25);
    return m;
  ...
  const livePos = live && (live.status === "locked" || live.status === "sim") ? toXY(live) : null;
      ...
      <div className="card relative overflow-hidden p-3">
        <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="mx-auto block w-full max-w-[380px]">
          ...
          <g stroke="rgba(122,143,196,0.14)" strokeWidth="1">
            {[0.25, 0.5, 0.75, 1].map((f) => <circle key={f} cx={C} cy={C} r={(SIZE / 2 - 20) * f} fill="none" />)}
            <line x1={C} y1="14" x2={C} y2={SIZE - 14} /><line x1="14" y1={C} x2={SIZE - 14} y2={C} />
          </g>
          <g className="radar-sweep">
            <path d={`M ${C} ${C} L ${C + (SIZE / 2 - 20) * Math.cos(-1.15)} ${C + (SIZE / 2 - 20) * Math.sin(-1.15)} A ${SIZE / 2 - 20} ${SIZE / 2 - 20} 0 0 1 ${C + (SIZE / 2 - 20) * Math.cos(-0.55)} ${C + (SIZE / 2 - 20) * Math.sin(-0.55)} Z`} fill="url(#sweepGrad)" opacity="0.55" />
          </g>
          <defs>
        ...
        <div className="absolute right-3 top-3 flex flex-col gap-1.5">
          {[{ l: "+", f: () => setZoom((z) => Math.min(3, z + 0.5)) }, { l: "−", f: () => setZoom((z) => Math.max(0.5, z - 0.5)) }].map((b) => (
            <button key={b.l} onClick={b.f} className="grid h-9 w-9 cursor-pointer place-items-center rounded-xl bg-white/10 font-display text-lg font-bold text-white backdrop-blur transition hover:bg-white/20 active:scale-90" aria-label={b.l === "+" ? "Perbesar" : "Perkecil"}>{b.l}</button>
          ))}
        </div>
      </div>
      {sel ? (
        <div className="card anim-fade-up flex items-center gap-3 p-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="flex items-center gap-1.5 text-[13px] font-extrabold text-ink-900">
              ...
              <Chip tone={sel.status === "REJECTED" ? "danger" : "ok"} className="!px-1.5 !py-0.5 !text-[9px]">{sel.status === "REJECTED" ? "DITOLAK" : sel.type}</Chip>
            </p>
            <p className="text-[11px] font-semibold text-ink-400">{wibShortDate(new Date(sel.ts))} · {wibTime(new Date(sel.ts))} · {formatMeters(sel.distanceM)} dari HQ</p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {liveDist !== null && <Chip tone={liveDist <= radiusM ? "ok" : "danger"}>Anda {formatMeters(liveDist)}{live?.simulated ? " · SIM" : ""}</Chip>}
