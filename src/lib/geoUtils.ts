 ...
 * Web equivalent of geo_utils.py (geopy).
 */
export interface GeoPoint { lat: number; lon: number; }

export type GeoStatus = "searching" | "locked" | "denied" | "unavailable" | "sim";

export interface GeoReading extends GeoPoint {
  accuracy: number;
  status: GeoStatus;
  ts: number;
  simulated: boolean;
...
/** Point at `bearing` degrees, `distM` meters from p — for the map radius handle. */
...
/** Rough bearing from a → b in degrees. */
